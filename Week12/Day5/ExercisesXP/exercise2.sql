...
select film_id, title, description, length, rental_rate from film where title in ('Good Will Hunting') ;
...
select film.title from film left join inventory on film.film_id = inventory.film_id where inventory.inventory_id is null order by film.film_id;
