...
while start == 'y':
    ...
    while move_count < 9:
        if move:
            ...
            player_one = input('make your turn (write row and column and divide it with space):\n').split(',')
            ...
            if gui_list[voc_row[player_one[0]]][voc_col[player_one[1]]] == player_one_word or \
                ...
                continue
            ra(player_one, player_one_word)
            move_count += 1
            player_one.clear()
            if win_condition(gui_list, player_one_word):
                win = True
                winner = 'Player 1'
                break
        else:
            ...
            if gui_list[voc_row[player_two[0]]][voc_col[player_two[1]]] == player_two_word or \
                ...
                print('this place is spotted, try again')
                continue
            ra(player_two, player_two_word)
            move_count += 1
            player_two.clear()
            if win_condition(gui_list, player_two_word):
                win = True
                winner = 'Player 2'
                break
        if move is True:
